...
# Excel Sheet 名称映射 (左边是代码用的key，右边是Excel里的Sheet名)
CHANNEL_SHEET_MAP = {
    "GOFO-报价": "GOFO-报价",
...
<div class="container my-4">
                    ...
                    <form id="calcForm">
                        <div class="row mb-4">
                            ...
                            <div class="col-md-2">
                                ...
                                <div class="form-check form-switch mt-2"><input class="form-check-input" type="checkbox" id="peakToggle"><label class="form-check-label" for="peakToggle">启用 (Peak)</label></div>
                            </div>
                            <div class="col-md-2">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-8">
                                ...
                                <div class="row g-2">
                                    ...
                                    <div class="col-6 mt-2"><select class="form-select" id="weightUnit"><option value="lb">lb (磅)</option><option value="kg">kg (千克)</option><option value="oz">oz (盎司)</option><option value="g">g (克)</option></select></div>
                                </div>
                            </div>
                        </div>
                        <div class="d-grid mt-4"><button type="button" class="btn btn-primary btn-calc btn-lg text-white" id="btnCalc">开始计算报价 (Calculate)</button></div>
                    </form>
                </div>
            </div>
        </div>
    ...
    <div class="row" id="resultSection" style="display:none;">
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center"><span>📊 报价结果 (Results)</span><span id="resTierBadge" class="badge bg-warning text-dark"></span></div>
                <div class="card-body">
                    <div class="alert alert-info py-2" id="pkgSummary"></div>
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover result-table">
                            <thead><tr><th width="15%">渠道</th><th width="5%">分区</th><th width="8%">计费重<br>(LB)</th><th width="10%">基础运费</th><th width="10%">燃油费</th><th width="10%">旺季费</th><th width="10%">住宅/其他</th><th width="10%">超规费</th><th width="12%">总费用($)</th><th width="10%">状态</th></tr></thead>
...
<footer><div class="container"><p>&copy; 2026 速狗海外仓报价系统 | Powered by GitHub Actions</p><small class="text-muted">仅供参考，实际费用以仓库账单为准。</small></div></footer>
...
<script>
    ...
    function getDimWeight(L, W, H, channel) {
        ...
        if (channel.toLowerCase().includes('fedex')) { if (vol < 1728) divisor = 400; else divisor = 250; }
        return vol / divisor;
    }
    ...
    document.getElementById('btnLookup').onclick = function() {
        ...
        if (!DATA.zip_db[zip]) { infoDiv.innerHTML = "<span class='text-danger'>❌ 未找到该邮编 (Zip Not Found)</span>"; CUR_ZONES = {}; return; }
    ...
    document.getElementById('btnCalc').onclick = function() {
        ...
        if (!DATA.tiers[tier]) { tbody.innerHTML = '<tr><td colspan="10" class="text-danger">无该等级数据 (请上传 Excel)</td></tr>'; return; }
        ...
        channels.forEach(ch => {
            ...
            else { basePrice = foundRow[zoneKey]; if (!basePrice) { status = "缺报价"; statusClass = "status-warn"; basePrice = 0; } }
            ...
            if (basePrice > 0) {
                ...
                if (isPeak) { if (isRes) peakFee += DATA.surcharges.peak_res; if (isOversize) peakFee += DATA.surcharges.peak_oversize; if (isUnauthorized) peakFee += DATA.surcharges.peak_unauthorized; }
...
def get_sheet_by_name(excel_file, target_name):
    """
    读取Excel的特定Sheet。
    关键修改：engine='openpyxl' 以解决格式识别错误
    """
    try:
        # ⚠️ 显式指定 engine='openpyxl' 是修复该错误的关键
        ...
        print(f"    > [WARN] 未找到 Sheet: {target_name} (可用Sheet: {xl.sheet_names})")
        return None
    except Exception as e:
        print(f"    > [ERROR] 读取 Excel 失败: {e} | 文件: {os.path.basename(excel_file)}")
        return None
...
def load_zip_db():
    ...
    if not os.path.exists(path):
        print(f"❌ 错误: 找不到文件 {path}。请确保已上传 T0.xlsx 到 data/ 目录")
        return {}
    ...
    try:
        ...
        for idx, row in df.iloc[start_row:].iterrows():
            ...
            if z.isdigit() and len(z) == 5:
                ...
                zip_db[z] = {
                    "s": str(row[3]).strip(), "c": str(row[4]).strip(),
                    "r": str(row[2]).strip(), "z": zones
                }
    except Exception as e:
        print(f"解析邮编数据出错: {e}")
        ...
        for ch_key, sheet_name in CHANNEL_SHEET_MAP.items():
            ...
            try:
                ...
                for idx, val in enumerate(headers):
                    if 'weight' in val or 'lb' in val or '重量' in val: weight_idx = idx
                    if 'zone' in val:
                        z = re.sub(r'[^\d]', '', val) 
                        if z: zone_map[z] = idx
