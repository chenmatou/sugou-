...
# --- 渠道配置表 ---
# allow_wh: 仓库白名单
# fuel_mode: 'manual'(手动/自动抓取), 'none'(无)
# fuel_discount: True 表示该渠道燃油费打85折
# sheet_col_offset: 'left'(强制取左栏), 'right'(强制取右栏), None(自动)
CHANNEL_CONFIG = {
    "GOFO-报价": {
        "keywords": ["GOFO", "报价"], 
    ...
    "FedEx-632-MT-报价": {
        "keywords": ["632"],
        "sheet_col_offset": "left", # 强制取左侧(住宅)表，避免读取到右侧商业表
...
<head>
  ...
  <title>报价助手 (V2026.5 Final)</title>
  ...
  <style>
    ...
    .card-header { background: #fff; border-bottom: 1px solid #eee; font-weight: 700; color: #2c3e50; padding: 15px 20px; }
    .price-main { font-size: 1.4rem; font-weight: 800; color: #2980b9; }
    .warn-box { background: #fff3cd; border: 1px solid #ffeeba; color: #856404; padding: 15px; border-radius: 6px; font-size: 0.9rem; margin-bottom: 20px; }
    .loc-tag { font-size: 0.85rem; background: #e8f4fd; color: #2980b9; padding: 4px 10px; border-radius: 20px; display: inline-block; margin-top: 5px; font-weight: 600; }
    .compliance-err { color: #c0392b; font-weight: 700; font-size: 0.85rem; }
...
<div class="header-bar">
  <div class="container d-flex justify-content-between align-items-center">
    <div><h4 class="m-0 fw-bold">📦 业务员报价助手</h4><div class="small opacity-75">V2026.5 | 地区中英文 | 85折修复</div></div>
    <div class="text-end d-none d-md-block"><span class="badge bg-info text-dark">T0-T3 实时</span></div>
  </div>
</div>
          ...
          <form id="calcForm">
            ...
            <div class="bg-light p-3 rounded border mb-3">
                <div class="d-flex justify-content-between align-items-center mb-1">
                    ...
                    <span class="badge bg-warning text-dark" style="font-size:0.7rem">MT系列生效</span>
                </div>
                <div class="input-group input-group-sm">
                    <input type="number" class="form-control fw-bold text-primary" id="fuelInput" value="16.0" step="0.01">
                    <span class="input-group-text">%</span>
                </div>
                <div class="form-text small text-muted mt-1">* 已自动抓取文档费率，支持手动修改。<br>FedEx-632/超大件自动应用85折。</div>
            </div>
            ...
            <div class="compliance-box" id="complianceBox" style="display:none;">
              ...
              <ul class="mb-0 ps-3 small" id="complianceList"></ul>
            </div>
      ...
      <div class="card h-100">
        ...
        <div class="card-body">
          <div class="warn-box">
            ...
            1. <b>燃油费</b>：依据左侧输入值计算。仅 <b>FedEx-632 / 超大包裹</b> 享受 <b>85折</b>。<br>
            2. <b>无报价</b>：若显示无报价，请检查“规格预检”是否超标，或该仓库不支持此渠道。<br>
            3. <b>邮编分区</b>：GOFO渠道使用自营分区；FedEx系列使用标准分区逻辑。<br>
            4. <b>USPS</b>：已取消旺季附加费。<br>
            5. <b>免责声明</b>：实际账单以运营商复核重量/尺寸及可能产生的偏远费为准，实报实销。
          </div>
...
<footer class="text-center py-4 text-muted small">
  &copy; 2026 SureGo Logistics | Updated: <span id="updateTime"></span>
</footer>
...
<script>
  ...
  document.getElementById('zipCode').addEventListener('blur', function() {
    ...
    if(zip.length === 5 && DATA.zip_db && DATA.zip_db[zip]) {
        ...
        // 显示格式：Los Angeles, CA (加利福尼亚)
        display.innerHTML = `<div class="loc-tag">📍 ${info.city}, ${info.state} (${info.cn_state})</div>`;
    }
  });
  ...
  function checkCompliance(pkg) {
    ...
    if (pkg.Wt > 150) msgs.push("超过150lb (仅XLmiles可发)");
  ...
  function updateComplianceUI() {
    ...
    if(L>0 && Wt>0) {
      ...
      let html = "";
      if(res.msgs.length > 0) html += `<li class="compliance-err">${res.msgs.join(', ')}</li>`;
  ...
  document.getElementById('btnCalc').onclick = () => {
    ...
    Object.keys(DATA.channels).forEach(chName => {
      ...
      // 查找 >= finalWt 的最小行 (修复：加微小偏移防止浮点误差)
      let row = priceTable.find(r => r.w >= finalWt - 0.001);
      
      if(!row) return; // 超重或无数据
    ...
    if(tbody.innerHTML === '') {
        tbody.innerHTML = `<tr><td colspan="7" class="text-center py-4 text-danger">无可用报价 (可能超重/超尺寸/仓库不支持)</td></tr>`;
    }
  };
</script>
    ...
    try:
        ...
        if start_row != -1 and zip_col != -1:
            for r in range(start_row+1, len(df)):
                try:
                    z = str(df.iloc[r, zip_col]).strip().split('.')[0].zfill(5)
                    ...
                    if len(z) == 5 and z.isdigit():
                        db[z] = {
                            "city": city,
                            "state": state,
                            "cn_state": STATE_MAP_CN.get(state, state)
                        }
                except: continue
        print(f"  [Info] Loaded {len(db)} ZIPs from GOFO.")
